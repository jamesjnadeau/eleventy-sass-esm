...
const debug = debug$0("EleventySass:DependencyMap");
const debugDev = debug$0("Dev:EleventySass:DependencyMap");
class DependencyMap {
    constructor() {
    ...
    update(dependant, dependencies) {
        if (this.dependenciesByDependant.has(dependant)) {
            ...
            if (currentDeps.size === dependencies.length &&
                dependencies.every(e => currentDeps.has(e))) { // If no change
...
export default new DependencyMap();
