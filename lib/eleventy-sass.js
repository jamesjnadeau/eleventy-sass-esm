...
const debug = debug$0("EleventySass");
const debugDev = debug$0("Dev:EleventySass");
    ...
    if (rev) {
        eleventyConfig.on("eleventy.beforeWatch", (queue) => {
            for (let inputPath of queue) {
                ...
                debugDev("Invalidated the revision hashes for %O because of the update of %O", dependants, normalizedPath);
            }
        });
    }
};
export default eleventySass;
